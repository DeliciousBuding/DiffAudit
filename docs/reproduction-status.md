...
12. `W-1 / DPDM` 当前也不再是“仓库有了但还没接”。`strong-v3` 现在已经有 CPU 诊断和 GPU three-shadow `max128` 结果，这进一步证明当前白盒主线的瓶颈不是资产，而是更大规模 GPU comparator 稳定性。
13. 当前 `DPDM` 的真正技术限制已从“环境阻塞”变成“GPU comparator 在大规模设置下仍不稳定”。也就是说，白盒防御下一步应当同时保留 `strong-v3 max128` 这条可用结果，并继续诊断更大规模 GPU 路径，而不是继续证明 checkpoint 可以被读取。
