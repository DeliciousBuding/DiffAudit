...
- `Finding NeMo + local memorization + FB-Mem` 的 intake/eligibility note 已建立，当前最值得推进的唯一目标变为判断是否要继续补一个 `activation-only canary` 实现草案
...
- 当前能说的话：
  ...
  - 新的 [2026-04-10-finding-nemo-activation-only-canary-sketch](../workspaces/white-box/2026-04-10-finding-nemo-activation-only-canary-sketch.md) 已把未来 `activation-only canary` 的实现形状写成草案，但当前仍未开始实现
- 当前不能说的话：
  - 还不能说白盒论文级复现成功
