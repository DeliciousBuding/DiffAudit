...
- `next step`: 保持 `stochastic-dropout = provisional G-1 (repeat-confirmed at GPU512)`；把 `SecMI` 固定为独立 corroboration line 而不是 blocked baseline；不再为 naive `PIA + SecMI` fusion 消耗预算；`SimA` 当前不放 GPU；灰盒下一步转到 `TMIA-DM protocol-ready CPU probe definition`，而不是立即 `SimA` reopen 或直接跳到 `MoFit`
...
12. 当前 `TMIA-DM` 已达到 `protocol-ready but not execution-released`；下一条非 GPU 任务应是定义并实现最小 `TMIA-DM protocol probe`
