...
- `next step`: 保持 `stochastic-dropout = provisional G-1 (repeat-confirmed at GPU512)`；把 `SecMI` 固定为独立 corroboration line 而不是 blocked baseline；不再为 naive `PIA + SecMI` fusion 消耗预算；`SimA` 当前不放 GPU；defended `TMIA-DM GPU256` 首个 rung 已完成，当前最短下一步应是补 defended `GPU256` repeat，而不是回退到环境排障、重开 `short_window`/naive fusion，或直接跳到 `MoFit`
...
12. 当前 `TMIA-DM` 已完成最小 `protocol probe`、一次 repeat、same-split 比较、late-window refine、两次正向 `GPU128` rung，以及两次正向 `GPU256` rung；结论是该线已成为稳定 GPU challenger，且 defended `GPU256` 首 rung 也已成立，下一条任务应是 defended `GPU256` repeat 或材料化总表
