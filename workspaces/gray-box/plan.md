...
- `next step`: 保持 `stochastic-dropout = provisional G-1 (repeat-confirmed at GPU512)`；把 `SecMI` 固定为独立 corroboration line 而不是 blocked baseline；不再为 naive `PIA + SecMI` fusion 消耗预算；`SimA` 当前不放 GPU；`TMIA-DM late-window` 的第一次 defense interaction 已经表明 challenger 被削弱但未被击穿，当前最短下一步应是做 defended repeat，而不是回退到环境排障、重开 `short_window`/naive fusion，或直接跳到 `MoFit`
...
12. 当前 `TMIA-DM` 已完成最小 `protocol probe`、一次 repeat、same-split 比较、late-window refine、两次正向 `GPU128` rung，以及两次正向 `GPU256` rung；结论是该线已成为稳定 GPU challenger，且第一条 defense interaction 已经成立，下一条任务应是 defended repeat
